...
bool DCF77::processBuffer(void) {	
    ...
    { 
      ...
      if (time.Minute%3 == 0) {weatherBuffer=0;keyBuffer=0;}
      if (time.Minute%3 == 2) {
        for (int j=0;j<40;j++) {
        ...
        Serial.println();
        for (int j=0;j<14;j++) {
            weatherBuffer = weatherBuffer | ((unsigned long long) ((filledBuffer>>j) & 1 ) << ((time.Minute%3)*14)+j);
        }
        for (int j=0;j<42;j++){
